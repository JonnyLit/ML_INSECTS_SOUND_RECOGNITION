...
//#define I2S_READ_LEN      (AUDIO_RECORD_SIZE / 10) // to work properly, I2S_READ_LEN must be chosen in such a way that AUDIO_RECORD_SIZE / I2S_READ_LEN equals an integer value
...
void i2s_record_and_notify(void *arg){
  ...
  char* i2s_read_buff = (char*)heap_caps_calloc(i2s_read_len, sizeof(char), MALLOC_CAP_8BIT);
  if (!i2s_read_buff) {
      ...
      if (deserializeJson(doc, decryptedMessage) == DeserializationError::Ok) {
        if (doc["invitation_id"] == esp32_device.invitation_id) { //+++++++++++++++++++++++++++++++
          ...
          if (doc["Connection_permission"] == "allowed" || doc["Connection_permission"] == "already_in") {
            ...
            esp32_device.device_id = doc["device_id"]; // Update device ID
    ...
    try {
		  if (deserializeJson(doc, decryptedMessage) == DeserializationError::Ok) {
		    if (doc["device_id"] == esp32_device.device_id){
		      esp32_device.communication_id = doc["communication_id"];
		      esp32_device.audio_data_topic_key = doc["audio_data_topic_key"];
...
void setup(){
  ...
  //go on with settings only if the the device is successfully subscribed
  while(esp32_device.ready_to_set_device_flag == false){
    delay(500);
  }
  ...
  audio_data = (char*)heap_caps_calloc(audio_data_size, sizeof(char), MALLOC_CAP_8BIT);
  if (!audio_data) {
    Serial.println("Failed to allocate memory for audio data in PSRAM");
    reportMemoryStats();
    return;

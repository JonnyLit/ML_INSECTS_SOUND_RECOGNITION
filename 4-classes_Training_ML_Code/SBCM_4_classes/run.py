...
#-------random 70-30 split--------#
def random_split(queen_absent_dir, queen_present_newly_accepted_dir, queen_present_original_dir, queen_present_rejected_dir, n_chunks, mode, n_outputs, num_batch_size, num_epochs, class_names, target_names):
    X_train, X_test, Y_train, Y_test = prep.make_random(queen_absent_dir, queen_present_newly_accepted_dir, queen_present_original_dir, queen_present_rejected_dir, n_chunks, mode)
    ...
    print(f"rounded_predictions: {rounded_predictions}")
    print(f"rounded_labels: {rounded_labels}")
    cnf_matrix = confusion_matrix(rounded_labels, rounded_predictions)
    np.set_printoptions(precision=2)
    mt.plot_confusion_matrix(cnf_matrix, classes=class_names, title='Confusion matrix:')

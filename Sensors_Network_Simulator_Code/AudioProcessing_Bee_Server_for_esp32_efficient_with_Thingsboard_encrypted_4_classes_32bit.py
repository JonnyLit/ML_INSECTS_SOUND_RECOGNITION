...
LOW_BATTERY_VOLTAGE = 3.1 # 3.1 Volts is considere low battery
...
current_classification_results_file = "Hives_Classes.txt"
...
class RaspberryPi:
    ...
    def __init__(self, device_model, device_id, devices_fixed_id_dict, num_current_devices, num_max_allowed_devices,
                 ...
                 max_waiting_time_for_receiving_audio, max_resending_request, time_between_a_series_of_audio_requests, last_audio_request_publish_time, iter_dashboard,
                 net_password, last_communication_epoch_dict, I2S_sample_rate, record_time, I2S_channel_num, I2S_sample_bits, max_hive_number_ever_registered, upload_dashboard_period,
                 max_time_without_communication_with_a_device, esp32_max_time_on, esp32_deep_sleep_time, update_thingsboard_led_status, caution_time_period_before_audio_request,
                 current_timer, subscription_in_progress_timer=0, telegram_notifications=True, dev_is_connected=False, receive_audios_folder_path="received_audio_files/",
                 ...
                 devices_to_make_an_audio_request_list=None, go_on_with_audio_requests=None,
                 counter_resend_request_done=0, current_handled_device_id=None, subscription_in_progress=False,
                 split_audio_flag=True, upload_data_on_thingsboard_dashboard=False, save_audios_and_binaries=True):
        """
        ...
        :param split_audio_flag: bool - when True, the esp32 devices will record 3sec audios that will be split from the raspberrypy in 1 sec audios
        ...
        :param telegram_notifications: bool - if it's True, every time is detected a 'queen_absent' or 'queen_present_rejected' state for any hive, a telegram notification will be sent to the chat
        ...
        :param upload_data_on_thingsboard_dashboard: bool - if it's True, the system data will be uploaded on the Thingsboard dashboard, so updating the dashboard periodically. Otherwise not if False.
        :param save_audios_and_binaries: bool - if it's True, it will be saved in the collect_audios_folder_path the audio data received by the ESP32s, and their related segmented files and spectrogram binaries. Otherwise not if False.
        :param upload_dashboard_period: int - frequency period of the dashboard update
        :param iter_dashboard: int - iterator of the dashboard aupload, that goes from 0 to upload_dashboard_period
        """
        ...
        self.split_audio_flag = split_audio_flag
        ...
        self.upload_data_on_thingsboard_dashboard = upload_data_on_thingsboard_dashboard
        self.save_audios_and_binaries = save_audios_and_binaries
        self.upload_dashboard_period = upload_dashboard_period
        self.iter_dashboard = iter_dashboard
...
def save_audio_from_payload(client, msg, raspberrypi3B):
    ...
    if is_wav_file(msg.payload):
        ...
        if simulation_esp32_flag is False:
            ...
            random_label = random.choice(["<queen_absent>", "<queen_present_newly_accepted>", "<queen_present_original>", "<queen_present_rejected>"])
...
"""
# PER FARE UN TEST VELOCE CON TEMPI BREVI
raspberrypi3B = RaspberryPi(
    ...
    esp32_max_time_on=40, #def 260
    esp32_deep_sleep_time=85, #def 570
    time_between_a_series_of_audio_requests=120, #def 600
    devices_infos_dict={},
    ...
    max_time_without_communication_with_a_device=200, #def 900
    split_audio_flag=True,
    I2S_sample_rate=22050, #def 22050
    record_time=5,
    I2S_channel_num=1,
    I2S_sample_bits=32,
    list_of_devices_to_put_to_sleep = [],
    ...
    caution_time_period_before_audio_request=25, #def 40
    current_timer=0,
    imminent_audio_request_led_flag_thingsboard=False,
    last_audio_request_publish_time = 0,
    max_hive_number_ever_registered = 20,
    upload_data_on_thingsboard_dashboard=True,
    save_audios_and_binaries=True,
    upload_dashboard_period=20,
    iter_dashboard=0
)
"""
...
raspberrypi3B = RaspberryPi(
    ...
    max_waiting_time_for_receiving_audio=5,
    communication_id=None,
    esp32_max_time_on=260, #def 260
    esp32_deep_sleep_time=570, #def 570
    time_between_a_series_of_audio_requests=600, #def 600
    devices_infos_dict={},
    current_invitation_id=None,
    devices_to_make_an_audio_request_list=[],
    audio_data_topic_key=None,
    ...
    collect_audios_folder_path="audio_files_collector/",
    last_communication_epoch_dict={},
    max_time_without_communication_with_a_device=900, #def 900
    split_audio_flag=True,
    I2S_sample_rate=22050, #def 22050
    record_time=5,
    I2S_channel_num=1,
    ...
    list_of_devices_hives_to_put_to_sleep = [],
    update_thingsboard_led_status = False,
    caution_time_period_before_audio_request=40, #def 40
    current_timer=0,
    imminent_audio_request_led_flag_thingsboard=False,
    last_audio_request_publish_time = 0,
    max_hive_number_ever_registered = 20,
...
def on_message_thingsboard(client_thingsboard, userdata, msg):
    ...
    if msg.topic.startswith('v1/devices/me/rpc/request/'):
        ...
        # THINGSBOARD DASHBOARD: SLEEP_HIVE SWITCHES
        if data['method'].startswith("sleep_Hive/"):
            ...
            if raspberrypi3B.upload_data_on_thingsboard_dashboard is False:
                print("if raspberrypi3B.upload_data_on_thingsboard_dashboard is False")
                #sleeping_hives = raspberrypi3B.get_list_of_hives_numbers_to_put_to_sleep()
                key_switch = "state_sleep_Hive/" + str(hive_number)
                key_led = "ledStatus" + str(hive_number)
                if data['params'] is True: # when it's True, the user attempted to switch the device to sleep
                    print("if data['params'] is True:")
                    switch_value = False # go back to previous state (if it was switched to True, then it means that it was previously False)
                    if hive_number in raspberrypi3B.get_list_of_hives_numbers_to_put_to_sleep():
                        led_value = 1 # ...then the led should be ON, since the device was activated
                    else:
                        led_value = 0  # ...then the led should be OFF, since the device was deactivated anyway
                else: # when it's False, the user attempted to switch on the device
                    print("if data['params'] is False:")
                    switch_value = True # go back to previous state (if it was switched to False, then it means that it was previously True)
                    if hive_number in raspberrypi3B.get_list_of_hives_numbers_to_put_to_sleep():
                        led_value = 0  # ...then the led should be OFF, since the device was deactivated
                    else:
                        led_value = 1  # ...then the led should be ON, since the device was activated

                thingsboard_sleep_Hive_switch_and_led = {key_switch: switch_value, key_led: led_value}
                #if hive_number not in sleeping_hives:
                #    thingsboard_sleep_Hive_switch_and_led = {key_switch: False, key_led: 0}
                #else:
                #    thingsboard_sleep_Hive_switch_and_led = {key_switch: True, key_led: 1}

                payload = json.dumps(thingsboard_sleep_Hive_switch_and_led)
            ...
            elif raspberrypi3B.upload_data_on_thingsboard_dashboard is True:
                print("elif raspberrypi3B.upload_data_on_thingsboard_dashboard is True")
                    ...
                    if hive_number not in raspberrypi3B.get_list_of_hives_numbers_to_put_to_sleep():
                        ...
                        for i in range(1, raspberrypi3B.max_hive_number_ever_registered + 1):
                            ...
                            if i in list_of_devices:
                                print(f"if i in list_of_devices:")
                                thingsboard_leds.update({key_led: 1})
                            else:
                                print(f"elif i not in list_of_devices:")
        ...
        if data['method'].startswith("set_state_Telegram_switch"):
            ...
            elif raspberrypi3B.upload_data_on_thingsboard_dashboard is False:
                thingsboard_telegram_notification = {"state_Telegram_switch": raspberrypi3B.telegram_notifications}
                payload = json.dumps(thingsboard_telegram_notification)
                # Publish data to the device telemetry topic
                result = client_thingsboard.publish(f'v1/devices/me/telemetry', payload, qos=1)
                # Check if the publish was successful
                if result.rc == mqtt.MQTT_ERR_SUCCESS:
        ...
        # THINGSBOARD DASHBOARD: UPDATE_DASHBOARD BUTTON
        if data['method'].startswith("set_state_Dashboard_switch"):
            print_debug("if data['method'].startswith('set_state_Dashboard_switch'):")
            if data['params'] is True:
                print_debug("if data['params'] is True:")
                raspberrypi3B.upload_data_on_thingsboard_dashboard = True
            elif data['params'] is False:
                print_debug("if data['params'] is False:")
                raspberrypi3B.upload_data_on_thingsboard_dashboard = False


        # THINGSBOARD DASHBOARD: SAVE_AUDIOS_AND_BINARIES BUTTON
        if data['method'].startswith("set_state_Save_audios_switch"):
            print_debug("if data['method'].startswith('set_state_Save_audios_switch'):")
            if raspberrypi3B.upload_data_on_thingsboard_dashboard is True:
                print("if raspberrypi3B.upload_data_on_thingsboard_dashboard is True:")
                if data['params'] is True:
                    print("if data['params'] is True:")
                    raspberrypi3B.save_audios_and_binaries = True
                elif data['params'] is False:
                    print("if data['params'] is False:")
                    raspberrypi3B.save_audios_and_binaries = False
            elif raspberrypi3B.upload_data_on_thingsboard_dashboard is False:
                print("if raspberrypi3B.upload_data_on_thingsboard_dashboard is False:")
                thingsboard_save_audios_switch = {"state_Save_audios_switch": raspberrypi3B.save_audios_and_binaries}
                payload = json.dumps(thingsboard_save_audios_switch)
                # Publish data to the device telemetry topic
                result = client_thingsboard.publish(f'v1/devices/me/telemetry', payload, qos=1)
                # Check if the publish was successful
                if result.rc == mqtt.MQTT_ERR_SUCCESS:
        ...
        # THINGSBOARD DASHBOARD: REFRESH DASHBOARD
        if data['method'].startswith("set_Dashboard_refresh"):
            print_debug("if data['method'].startswith('set_Dashboard_refresh'):")
            raspberrypi3B.upload_data_on_thingsboard_dashboard = True


        """
        # THINGSBOARD DASHBOARD: UPLOAD LAST DATA
        if data['method'].startswith("set_Upload_last_data"):
            print_debug("if data['method'].startswith('set_Upload_last_data'):")
            raspberrypi3B.upload_data_on_thingsboard_dashboard = True

            thingsboard_system_status_table = []
            thingsboard_alarms = []
            ...
            for device_id, info in raspberrypi3B.devices_infos_dict.items():
                #merged_dict = {}
                # Recupera le informazioni di tutti i dispositivi connessi, così da inserirle nella tabella "System Status" della dashboard di Thingsboard
                thingsboard_system_status_table.append({"Device_model": info['device_model'], "Hive": info['location'], "State": info['state'], "Battery_voltage": info['battery_level']})
                #print(f"thingsboard_system_status_table: {thingsboard_system_status_table}")

                # Formazione degli allarmi da mostrare sulla dashboard di Thingsboard
                if info['state'] == "queen_absent" or info['state'] == "queen_present_rejected" or float(info['battery_level']) <= LOW_BATTERY_VOLTAGE:
                    thingsboard_alarms.append({"Device_model_alarm": info['device_model'], "Hive_alarm": info['location'], "State_alarm": info['state'], "Battery_voltage_alarm": info['battery_level']})
                    alarm_notification = True

            # send data only when raspberrypi3B.iter_dashboard is between [4 and 11]
            while raspberrypi3B.iter_dashboard < 4 or raspberrypi3B.iter_dashboard > 12:
                print("while raspberrypi3B.iter_dashboard < 4 or raspberrypi3B.iter_dashboard > 12:")
                time.sleep(1)

            # THINGSBOARD STATUS TABLE
            print("for device_status_dict in thingsboard_system_status_table:")
            for device_status_dict in thingsboard_system_status_table:
                payload = json.dumps(device_status_dict)
        ...
        # THINGSBOARD DASHBOARD: SEND CSV
        if data['method'].startswith("set_Send_csv"):
            print_debug("if data['method'].startswith('set_Send_csv'):")
            if raspberrypi3B.upload_data_on_thingsboard_dashboard is True:
                print("if raspberrypi3B.upload_data_on_thingsboard_dashboard is True:")
                ...
                with open(file_path, 'rb') as file:
                    ...
                    data = {
                        'chat_id': CHAT_ID,
                        'filename': 'file.csv'  # Optional: specify a filename
                    }
                    response = requests.post(url, files=files, data=data)
                # Print response to see if it succeeded
                print(response.json())


        # THINGSBOARD DASHBOARD: RESET CSV
        if data['method'].startswith("set_Reset_csv"):
            print_debug("if data['method'].startswith('set_Reset_csv'):")
            if raspberrypi3B.upload_data_on_thingsboard_dashboard is True:
                print("if raspberrypi3B.upload_data_on_thingsboard_dashboard is True:")
        ...
        # THINGSBOARD DASHBOARD: RECORD TIMINGS
        if data['method'].startswith("set_time_min/"):
            print_debug("if data['method'].startswith('set_time_min/'):")

            if raspberrypi3B.upload_data_on_thingsboard_dashboard is False:
                print("if raspberrypi3B.upload_data_on_thingsboard_dashboard is False:")
                #In questo caso la dashboard di Thingsboard non deve essere aggiornata,
                # perciò i led dei tempismi devono mantenere il loro precedente stato
                ...
                if raspberrypi3B.time_between_a_series_of_audio_requests == ten_min_in_sec:
                    led10min = True
                    led30min = False
                    led1hour = False
                elif raspberrypi3B.time_between_a_series_of_audio_requests == thirty_min_in_sec:
                    led10min = False
                    led30min = True
                    led1hour = False
                elif raspberrypi3B.time_between_a_series_of_audio_requests == one_hour_in_sec:
                    led10min = False
                    led30min = False
                    led1hour = True
                else: # default 10 min
...
def run_cpp_executable_for_inference_classification():
    ...
    num_classes = 4 # number of the classification output classes
    save_data_flag = 0 # 0 if raspberrypi3B.save_audios_and_binaries is False, 1 if True
    if raspberrypi3B.save_audios_and_binaries is True:
        save_data_flag = 1
...
def delete_files(source_folder):
    # Loop through all files in the source folder
    for filename in os.listdir(source_folder):
...
def split_audio_into_segments(input_folder, output_folder, filenames, segment_duration_sec=1):
    print("split_audio_into_segments")
    """
    Splits audio files into 32-bit PCM segments of a specified duration, discarding segments shorter than the specified duration.
    Saves segments in PCM 32-bit float format.

    Args:
        input_folder (str): Path to folder containing input WAV files.
        output_folder (str): Path to folder to save segmented files.
        ...
        if filename.endswith(".wav"):
            ...
            try:
                ...
                for i in range(0, len(y), segment_samples):
                    ...
                    # Optional: normalize the segment
                    #segment_normalized = librosa.util.normalize(segment)

                    # Convert to float32 for PCM 32-bit
                    #segment_float32 = segment_normalized.astype(np.float32)
                    ...
                    # Save segment in PCM 32-bit float format
                    sf.write(output_path, segment_float32, sr, subtype='PCM_32')
                    print(f"Created segment {i // segment_samples + 1} of {filename} and saved to {output_filename}")
                    segment_counter += 1
                    #if segment_counter >= 5:
                    #    break  # Only 3 segments per file
...
def get_cleaned_name(filename, labels_to_remove):
    name = filename
    # Remove specified labels
    for label in labels_to_remove:
        # Escape label in case it contains special regex characters
        escaped_label = re.escape(label)
        name = re.sub(escaped_label, '', name)
    # Remove segment identifiers like "_segment0", "_segment123", etc.
    name = re.sub(r'_segment\d+', '', name)
    # Remove any extension (anything after the last dot)
    name = re.sub(r'\.[^.]*$', '', name)
    return name


def update_Hive_states(original_filenames, file_with_classification_results, folder_with_segment_files, RaspberryPi):
    """
    Updates the Hive states by reading the provided file and storing its contents in a dictionary.
    Creates the file if it doesn't already exist.
    """
    print("update_Hive_states")


    # Gli ESP32 sono simulati, quindi gli stati dei dispositivi verranno aggiornati
    # in base ai label di classificazione riportati nei noi dei file all'interno
    # della cartella "receive_audios_folder_path", i quali contengono sia l'Hive (location)
    # che il label di classificazione (generato random in save_audio_from_payload)
    if simulation_esp32_flag is True:
        print("if simulation_esp32_flag is True:")
        ...
        try:
            print(f"original_filenames: {original_filenames}")
            result_dict = {}
            for filename in original_filenames:
                print(f"filename: {filename}")
                # Find the key inside parentheses
                start_key = filename.find('(')
                end_key = filename.find(')')
                # Find the value inside angle brackets
                start_value = filename.find('<')
                end_value = filename.find('>')

                if start_key != -1 and end_key != -1 and start_value != -1 and end_value != -1:
                    hive = filename[start_key + 1:end_key] # key
                    # Check if the key exists

                    if hive in result_dict:
                        pass
                    else:
                        classif_result = filename[start_value + 1:end_value] # value
                        result_dict[hive] = classif_result
                        device_id = get_device_id_from_location(hive, RaspberryPi)
                        print(f"device_id: {device_id}")
                        RaspberryPi.update_beehive_state(device_id, classif_result)
    ...
    elif simulation_esp32_flag is False:
        print("elif simulation_esp32_flag is False:")
        # leggere il file txt "file_with_classification_results", che contiene, per ogni riga,
        # una coppia key:value --> hive:classif_result per ogni file già classificato
        # dal file C++ Arm NN, classificazione effettuata dalla chiamata di sistema
        # effettuata da "run_cpp_executable_for_inference_classification()"
        ...
        try:
            ...
            with open(file_with_classification_results, 'r') as file:
                for line in file:
                    # Remove any leading/trailing whitespace and newline characters
                    line = line.strip()
                    if line:
                        # Split the line into key and value at the colon
                        hive, classif_result = line.split(':', 1)
                        # Store in the dictionary
                        result_dict[hive.strip()] = classif_result.strip()
                        device_id = get_device_id_from_location(hive, RaspberryPi)
                        print(f"device_id: {device_id}")
                        RaspberryPi.update_beehive_state(device_id, classif_result)
...
def main():
    ...
    try:
        ...
        raspberrypi3B.iter_dashboard = 0
        upload_dashboard_period = raspberrypi3B.upload_dashboard_period  # every 30 sec we upload the dashboard, except the 30 sec before the audio request
        raspberrypi3B.imminent_audio_request_led_flag_thingsboard = False
        ...
        try:
            ...
            while True:
                ...
                print_debug(f"raspberrypi3B.iter_dashboard = {raspberrypi3B.iter_dashboard:.2f}")
                ...
                print(f"if {raspberrypi3B.iter_dashboard} >= {upload_dashboard_period} and {raspberrypi3B.current_timer} < {(raspberrypi3B.time_between_a_series_of_audio_requests-raspberrypi3B.caution_time_period_before_audio_request)}:")
                if raspberrypi3B.iter_dashboard >= upload_dashboard_period and raspberrypi3B.current_timer < (raspberrypi3B.time_between_a_series_of_audio_requests-raspberrypi3B.caution_time_period_before_audio_request) and raspberrypi3B.imminent_audio_request_led_flag_thingsboard is False:
                    print("ENTER: if raspberrypi3B.iter_dashboard >= upload_dashboard_period and raspberrypi3B.current_timer < (raspberrypi3B.time_between_a_series_of_audio_requests-raspberrypi3B.caution_time_period_before_audio_request) and raspberrypi3B.imminent_audio_request_led_flag_thingsboard is False:")
                    ...
                    if raspberrypi3B.upload_data_on_thingsboard_dashboard is True:
                        ...
                        # THINGSBOARD DASHBOARD: UPDATE_DASHBOARD BUTTON
                        upload_data_on_thingsboard_dashboard_flag = raspberrypi3B.upload_data_on_thingsboard_dashboard
                        thingsboard_upload_data_on_thingsboard_dashboard = {"state_Dashboard_switch": upload_data_on_thingsboard_dashboard_flag}
                        print(f"upload_data_on_thingsboard_dashboard_flag: {upload_data_on_thingsboard_dashboard_flag}")

                        # THINGSBOARD DASHBOARD: SAVE_AUDIOS_AND_BINARIES BUTTON
                        save_audios_switch_flag = raspberrypi3B.save_audios_and_binaries
                        thingsboard_save_audios_switch = {"state_Save_audios_switch": save_audios_switch_flag}
                        print(f"save_audios_switch_flag: {save_audios_switch_flag}")
                        ...
                        thingsboard_message.update(thingsboard_upload_data_on_thingsboard_dashboard)
                        thingsboard_message.update(thingsboard_save_audios_switch)
                    ...
                    if raspberrypi3B.current_timer <= (raspberrypi3B.time_between_a_series_of_audio_requests-raspberrypi3B.caution_time_period_before_audio_request) and raspberrypi3B.imminent_audio_request_led_flag_thingsboard is True:
                        print("if raspberrypi3B.iter_dashboard >= upload_dashboard_period and raspberrypi3B.imminent_audio_request_led_flag_thingsboard is True:")
                    ...
                    raspberrypi3B.iter_dashboard += 1
                ...
                if raspberrypi3B.current_timer >= raspberrypi3B.time_between_a_series_of_audio_requests:
                    ...
                    if len(raspberrypi3B.devices_to_make_an_audio_request_list) > 0:
                        ...
                        if raspberrypi3B.split_audio_flag is True:
                            print("if raspberrypi3B.split_audio_flag is True:")
                        ...
                        if num_audio_files > 0:
                            ...
                            file_with_classification_results = raspberrypi3B.receive_audios_folder_path + current_classification_results_file
                            update_Hive_states(filenames, file_with_classification_results, raspberrypi3B.receive_audios_folder_path, raspberrypi3B)
                            ...
                            alarm_notification = False
                            telegram_message = ""
                            telegram_message_queen_absent = "No queen detected at the hives: "
                            alarm_notification_queen_absent = False
                            telegram_message_new_queen_not_accepted = "The queen has not been accepted at the hives: "
                            alarm_notification_queen_present_rejected = False
                            telegram_message_low_battery = f"Low battery (<{LOW_BATTERY_VOLTAGE}V) at the hives: "
                            alarm_notification_low_battery = False
                            ...
                            for device_id, info in raspberrypi3B.devices_infos_dict.items():
                                ...
                                # Formazione dei messaggi di telegram per ogni caso
                                if info['state'] == "queen_absent":
                                    print("if info['state'] == 'queen_absent':")
                                    alarm_notification_queen_absent = True
                                    telegram_message_queen_absent += info['location'] + ','
                                elif info['state'] == "queen_present_rejected":
                                    print("elif info['state'] == 'queen_present_rejected':")
                                    alarm_notification_queen_present_rejected = True
                                    telegram_message_new_queen_not_accepted += info['location'] + ','

                                if float(info['battery_level']) <= LOW_BATTERY_VOLTAGE:
                                    print("if info['battery_level'] <= LOW_BATTERY_VOLTAGE:")
                                    print(f"if {info['battery_level']} <= {LOW_BATTERY_VOLTAGE}:")
                                    alarm_notification_low_battery = True
                                    telegram_message_low_battery += info['location'] + ','
                            ...
                            # Unione dei messaggi di telegram da inviare in una sola volta
                            if alarm_notification_queen_absent is True:
                                print("if alarm_notification_queen_absent is True:")
                                telegram_message_queen_absent = telegram_message_queen_absent[:-1] + '.'
                                alarm_notification = True
                                telegram_message += telegram_message_queen_absent
                            if alarm_notification_queen_present_rejected is True:
                                print("if alarm_notification_queen_present_rejected is True:")
                                telegram_message_new_queen_not_accepted = telegram_message_new_queen_not_accepted[:-1] + '.'
                                alarm_notification = True
                                if alarm_notification_queen_absent is True:
                                    print("if alarm_notification_queen_absent is True:")
                                    telegram_message += "\n" + telegram_message_new_queen_not_accepted
                            if alarm_notification_low_battery is True:
                                print("if alarm_notification_low_battery is True:")
                                telegram_message_low_battery = telegram_message_low_battery[:-1] + '.'
                                alarm_notification = True
                                if alarm_notification_queen_absent is True or alarm_notification_queen_present_rejected is True:
                                    print("if alarm_notification_queen_absent is True or alarm_notification_queen_present_rejected is True:")
                                    telegram_message += "\n" + telegram_message_low_battery

                            print(f"thingsboard_alarms: {thingsboard_alarms}")
                            print(f"telegram_message: {telegram_message}")

                            # Update della tabella "System Status" della dashboard di Thingsboard
                            ...
                            if alarm_notification is True and raspberrypi3B.telegram_notifications is True:
                                result = send_telegram_message(CHAT_ID, telegram_message, BOT_TOKEN)
                                print(result)
                            ...
                            if raspberrypi3B.upload_data_on_thingsboard_dashboard is True:
                                ...
                                # Thingsboard update System Status Table
                                ...
                                for device_status_dict in thingsboard_system_status_table:
                                    print("for device_status_dict in thingsboard_system_status_table:")
                                    print(f"device_status_dict: {device_status_dict}")
                                    payload = json.dumps(device_status_dict)
                                    print(f"payload: {payload}")
                                    # Publish data to the device telemetry topic
                                    result = client_thingsboard.publish(f'v1/devices/me/telemetry', payload, qos=1)
                            ...
                            if raspberrypi3B.save_audios_and_binaries is False:
                                delete_files(raspberrypi3B.receive_audios_folder_path)
                            elif raspberrypi3B.save_audios_and_binaries is True:
                                ...
                                move_files(raspberrypi3B.receive_audios_folder_path, raspberrypi3B.collect_audios_folder_path)  # move the files from the previous audio_folder to the collector_folder, in order to empty the audio_folder for the next preprocess operations

...
class Device:
    ...
    def __init__(self, device_model, device_id, location, battery_level, deep_sleep_time, fixed_id, deep_sleep_simulation_time=42, deep_sleep_simulation_flag=False, record_time=1, reset_flag = False, max_time_on=600, max_time_for_successful_subscription=10, epoch_time_at_subscription=None, new_audio_data_is_available=False, currently_sending_audio=False, start_recording_now=False, start_sending_audio_now=False, enable_audio_record=False, net_password="", subscribed_flag=False, last_record_date=None, last_record_epoch=None, audio_data_topic_key=None, communication_id=None, invitation_id=None, state=None, ip_address=None):
        """
...
def publish_infos_with_invitation_id(client, topic, Device, invitation_id):
    ...
    infos_content = Device.all_infos_to_dict()
    message = {"infos":infos_content, "invitation_id": invitation_id, "net_password":Device.get_net_password(), "simulation": True}
    message = json.dumps(message)
...
def set_device_default_values(Device):
        ...
        Device.fixed_id="f1-dsfagafvbcv"
        Device.location="Hive1"
        Device.battery_level=3.6
        Device.state="idle"
...
#def on_message(client, userdata, msg, esp32_device):
        ...
        try:
            if json_received["invitation_id"] == esp32_device.get_invitation_id():
                ...
                if json_received["Connection_permission"] == "allowed" or json_received["Connection_permission"] == "already_in":
                    print_debug("if json_received[Connection_permission] == allowed or json_received[Connection_permission] == already_in:")
                    #if json_received["Connection_permission"] == "allowed" or json_received["Connection_permission"] == "already_in":
                    ...
                    print_debug(f"current_raspberrypi_timer: {current_raspberrypi_timer} and deep_sleep_time: {deep_sleep_time}")
    ...
    # elif msg.topic == "audio/payload" + topic_key:                # only for the RaspberryPy
        ...
        try:
            if json_received["device_id"] == esp32_device.get_device_id():
                esp32_device.update_communication_id(json_received["communication_id"])
                publish_infos_with_communication_id(client, "Raspberry_get_infos", esp32_device, json_received["communication_id"])
        except json.JSONDecodeError:
            print_debug("Received payload is not valid JSON.")
...
def main():
    ...
    publish_infos_with_invitation_id(client, "Raspberry_connect_to_sensors_net/request", esp32_device, generate_random_code(20))
            ...
            if time.time() - time_last_loop >= 1:
                ...
                if esp32_device.get_subscribed_flag() is True:
                    ...
                    if time.time() - esp32_device.get_epoch_time_at_subscription() >= esp32_device.get_max_time_on():
                        ...
                        print(f"THE DEVICE IS ON FOR TOO LONG: {time.time() - esp32_device.get_epoch_time_at_subscription()} seconds.")
                ...
                if esp32_device.get_subscribed_flag() is False:
                    ...
                    if time.time() - esp32_device.get_epoch_time_at_subscription() >= esp32_device.get_max_time_for_successful_subscription():
                        # Set up the timer wakeup
                        print(f"THE DEVICE IS ON BUT NOT SUBSCRIBED FOR TOO LONG: {time.time() - esp32_device.get_epoch_time_at_subscription()} seconds.")
                ...
                if esp32_device.get_start_sending_audio_now() is True:
                    ...
                    esp32_device.update_new_audio_data_is_available(False)  # since we sent the last audio recorded, a new audio data must be recorded
                    esp32_device.update_start_recording_now(False) # no more recordings, the device should go on deep sleep mode
                ...
                if esp32_device.get_start_recording_now() is True and esp32_device.get_currently_sending_audio() is False and esp32_device.get_new_audio_data_is_available() is False:
                    print("if esp32_device.start_recording_now is True && esp32_device.currently_sending_audio is False)")

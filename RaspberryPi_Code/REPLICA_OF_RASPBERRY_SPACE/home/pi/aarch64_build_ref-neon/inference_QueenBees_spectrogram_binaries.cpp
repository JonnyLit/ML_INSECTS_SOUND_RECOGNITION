...
int RunInference(const std::string& modelPath, std::vector<float> inputData, const std::string& backendType, const unsigned int rows, const unsigned int cols) {
    ...
    */
	...
	return predicted_class; // return the classification result (1 for 'queen', 0 for 'noqueen')
}
...
int main(int argc, char** argv) {
    ...
    std::cout << "" << std::endl;
    std::cout << "" << std::endl;
    std::cout << "-----------------------inference_QueenBees_spectrogram_binaries_exe-------------------------" << std::endl;
    ...
    for (const auto& entry : fs::directory_iterator(directoryPath)) {
        if (entry.is_regular_file() && entry.path().extension() == ".bin") {
	        ...
	        std::string old_bin_filename = filenameWithoutExtension + ".bin";
	        std::string old_wav_filename = filenameWithoutExtension + ".wav";
	        std::cout << "File to classify: "<< old_bin_filename << std::endl;
	        std::cout << "old_bin_filename: "<< old_bin_filename << std::endl;
            std::cout << "old_wav_filename: "<< old_wav_filename << std::endl;
            std::cout << "filenameWithoutExtension: "<< filenameWithoutExtension << std::endl;
            
        	// Check if the substring is present in the filename
            std::string substringToFind = "_seg"; // The predicted_class label must be inserted before this substring in the .wav and .bin filename
        	std::size_t pos = filenameWithoutExtension.find(substringToFind);
        	if (pos == std::string::npos) {
        		// if the substring "_seg" has not been found, then i have to insert the label right before the substring '.bin'
            	substringToFind = ".bin";
                std::cout << "substringToFind: "<< substringToFind << std::endl;
            	// Check if the substring is present in the filename
        		pos = old_bin_filename.find(substringToFind);
            }else{
                std::cout << "substringToFind: "<< substringToFind << std::endl;
            }
            ...
            */
            ...
            predicted_class = RunInference(modelPath, image, backendType, (unsigned int)rows, (unsigned int)cols);
            ...
            std::string new_filename = filenameWithoutExtension;
            std::string str_directoryPath = directoryPath;
			if (predicted_class == 1){
				//predicted_class == 1 means 'queen' presence
                std::cout << "if (predicted_class == 1){" << std::endl;
				std::string substringToInsert = "_queen"; // label to insert in the filename
				if (pos != std::string::npos) {
                	// Substring found, insert the new substring just before it
                	new_filename.insert(pos, substringToInsert);
            	}
                std::cout << (str_directoryPath+"/"+old_bin_filename).c_str() << " must be renamed to: " << (str_directoryPath+"/"+new_filename + ".bin").c_str() << "\n";
				// Renaming the .bin file with the label added in the name
				if (std::rename((str_directoryPath+old_bin_filename).c_str(), (str_directoryPath+new_filename + ".bin").c_str()) == 0) {
					std::cout << "File renamed successfully from " << old_bin_filename << " to " << new_filename + ".bin" << "\n";
				} else {
					perror("Error renaming .bin file"); // Output error if renaming fails
				}
				std::cout << (str_directoryPath+"/"+old_wav_filename).c_str() << " must be renamed to: " << (str_directoryPath+"/"+new_filename + ".wav").c_str() << "\n";
				// Renaming the .wav file with the label added in the name
				if (std::rename((str_directoryPath+"/"+old_wav_filename).c_str(), (str_directoryPath+"/"+new_filename + ".wav").c_str()) == 0) {
					std::cout << "File renamed successfully from " << old_wav_filename << " to " << new_filename + ".wav" << "\n";
				} else {
					perror("Error renaming .wav file"); // Output error if renaming fails
				}
			}else if (predicted_class == 0){
            	//predicted_class == 0 means 'noqueen' presence
                std::cout << "else if (predicted_class == 0){" << std::endl;
            	std::string substringToInsert = "_noqueen"; // label to insert in the filename
				if (pos != std::string::npos) {
                	// Substring found, insert the new substring just before it
                	new_filename.insert(pos, substringToInsert);
            	}
